...
As explained in Subsection \ref{subsec:ldpc}, each non-zero element of the \gls{bg} is replaced by a circularly shifted identity matrix.
%
...
To exemplify we show the cases of 3, 5 and 6 layers.
%
...
%
This mapping is specification-transparent, since the \gls{ue} can assume that the \glspl{dmrs} is precoded hand in hand with the data as shown in Figure \ref{fig:precoder-dmrs}.
%
...
%
The quantities more relevant to this work in the \gls{csi} report are \cite{AliZaidi632018}:
%
\begin{enumerate}
    ...
    \item \Gls{cqi}: Index to the preferred \gls{mcs}, which is the highest \gls{mcs} that gives a \gls{bler} of 10\%, given the selected precoder and rank.
\end{enumerate}
