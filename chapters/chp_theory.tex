...
\begin{itemize}
	...
	\item Code-block segmentation: Segments the transport block if it is larger than the supported by the \gls{ldpc} encoder. Produces \gls{cb} of equal size.
	\item Per-\gls{cb} \gls{crc} Attachment: A \gls{crc} is calculated and appended to each \gls{cb}.
	\item \gls{ldpc} Encoding: The solution used in \gls{nr} is a Quasi-cyclic \gls{ldpc} with two base graphs, that are used to built the different parity-check matrices with different payloads and rates.
...
The \gls{pmi} indicates the precoder matrix that the \gls{ue} believes it is the best option from a set of precoders, the codebook.
%
...
%
The \gls{pmi} only indicates the precoder that the \gls{ue} prefers, it imposes no restriction whatsoever on the precoder selection from the network for downlink transmissions \cite{ErikDahlman5G, AliZaidi632018}.
%
